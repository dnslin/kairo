...
T23 补充合同（用户于 2026-09-09 确认）：无附件且 trim 后为空的消息只保留原始记录，不入批、不计条数、不刷新静默、不回复。其余正文原样保存。计时使用 T22 持久化的观察时间；同 context 取得数据库锁后，先结束已到截止的旧批，再接入新消息。恰好截止归下一批，迟到的定时器不能延长窗口；重启沿用两个绝对截止，已过期立即结束，已被 `/new` 废弃的批次不恢复。
...
10 条和 30,000 字的边界均允许。字数按 Unicode 码点累计每条原文，包含非空正文的空格和换行，不包含系统拼接分隔符；中文与单码点表情各计一，组合表情按组成码点累计。不按 UTF-8 字节或 JavaScript UTF-16 长度计数。超过即立即结束并拒绝当前整批，后续普通消息另起一批；不以空 task 代替 collecting batch。
...
附件立即结束并拒绝当前整批，附件原因优先于超限，只发一次对应固定提示。先文字后附件时两者一起拒绝；先附件后文字时先拒绝附件，后续文字新开一批。仅检查 Driver 已提供的 file/image/voice 类别及 fileInfo、非空 images 元数据，不解析 raw、读取引用或下载媒体。拒绝提示复用 T21 的持久发送意图与恢复预算。
...
`waiting_for_user`、`ready_to_send`、`sending` 继续阻塞本会话后续任务。全局名额只在实际执行及 Tool 必要收尾结束后释放，不因业务任务已取消或超时而提前释放。取消后永久不退出的执行明确记录诊断，保留实际占用并交人工处理；不伪造 running、不突破并发、不自动重启。
...
- 默认空闲严格超过 2 小时才创建新 thread；恰好 2 小时继续原 thread（T24 终端确认）；
- 创建新 thread 不删除旧记录。
...
等待员工不消耗四分钟执行预算：进入 waiting_for_user 时保存剩余执行毫秒数及十分钟绝对等待截止。员工同意先持久化，任务仍阻塞本会话；拿到全局名额才恢复同一 running 任务，以“实际恢复时刻 + 剩余预算”设置下一段执行绝对截止。等名额也不扣执行预算，及时回答不能因随后等位被误判员工等待超时。拒绝或新问题替换旧等待时为 cancelled，未回答的等待到期为 timed_out。当前等待由持久 waitId 明确定位，不按同毫秒 UUID 顺序猜测。重启本身不延长任何期限。

Agent 在 10 秒内完成时直接回复。超过 10 秒只发送一次“正在查询企业知识，请稍候”，之后只在完成、失败或取消时通知。十秒按同任务各段执行时间累计，排除员工等待及等名额，包含 Agent/Tool 必要收尾；恰好十秒不提示。执行预算在锁内进入 waiting 时暂停，不把 COMMIT 回执延迟重新计算为运行时间。
...
T25 的进度通知可在等待员工期间暂停。仅当当前活调用在交付锁内明确尚未调用 Driver、并且只是因为任务进入 waiting_for_user 时，才恢复本次调用占用前的意图状态和预算；恢复后仍使用原 operationId/purpose。已触发、unknown、崩溃恢复和 CAS 败方不能按该规则退还预算或复活终态。等待问题通知也必须在实际交付锁内核对同一当前 waitId、未回答及未到截止，避免发送已失效的问题。
...
| 非私聊消息 | 无回复 | 只记录必要诊断 |
| Bot 自己的消息 | 无回复 | 直接忽略 |
| 输入超过 10 条或 30,000 字 | 内容过长，请缩短问题；文件处理功能将在后续阶段提供 | 不截断、不调用 Agent |
