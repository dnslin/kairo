...
`NormalizedModelTierInput` 只包含分类所需的本地事实：规范化文本，以及附件的媒体模态、可用文本表示和是否需要视觉理解等能力信息。附件能力信息必须来自确定性的本地解析、可信元数据或显式配置，不得由额外模型推断。相同规范化输入和同一规则版本必须产生相同 Tier。

规则按以下固定优先级执行：

1. **视觉能力优先**：只要当前请求需要理解图片内容、图表、版面或其他不能由现有文本表示完整承载的视觉信息，就选择 `VISION`。附件存在本身不是条件；纯文本文件，以及已经具有完整可信文本表示且当前任务不需要视觉信息的附件，不得仅因是附件而进入 `VISION`。
2. **复杂文本规则**：没有视觉需求时，先匹配显式 `DEEP` 规则。代表性类别包括代码分析、复杂推理和多步骤方案综合；具体规则表由后续实施配置维护，本规格不锁定关键词清单或长度阈值。
...
- 需要理解图片内容、图表或版面的请求优先进入 `VISION`；纯文本文件或无需视觉理解且已有完整文本表示的附件不因附件存在而进入 `VISION`；
- 无视觉需求时，明确的复杂文本请求进入 `DEEP`，简单请求进入 `FAST`，冲突时按 `DEEP` 规则优先于 `FAST` 规则处理；
