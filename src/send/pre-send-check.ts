...
/**
 * 发送前安全校验：单次 CDP 调用确保回复发送到正确的会话且消息上下文未变化
...
export async function preSendCheck(
  ...
  const state = await locator.checkPreSendState(
    target.content,
    target.sender,
    abortOnNewMessages
  );
  ...
  // 1. 检查会话是否切换
  if (state.activeSessionId !== target.sessionId) {
    const detail = `当前会话: ${state.activeSessionId ?? '未知'}, 目标会话: ${target.sessionId}`;
    log.warn({ activeSessionId: state.activeSessionId, targetSessionId: target.sessionId }, '会话已切换，中止发送');
  ...
  // 3. 检查是否有新消息
  if (abortOnNewMessages && state.hasNewMessages) {
