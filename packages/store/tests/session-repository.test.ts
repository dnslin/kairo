...
describe('SessionRepository 会话状态持久化仓储测试 (TDD Red -> Green)', () => {
  ...
  describe('1. sessions 表结构与索引约束验证', () => {
    it('应正确创建 sessions 表及其所有必需字段与数据类型', async () => {
      ...
      expect(Number(columnMap.get('id')?.pk)).toBe(1);
    ...
    it('应创建 idx_sessions_employee 与 idx_sessions_last_msg 等索引', async () => {
      const res = await db.execute('PRAGMA index_list(sessions)');
      const indexList = res.rows as unknown as Array<{
        seq: number;
        name: string;
        unique: number;
        origin: string;
      ...
      const indexNames = indexList.map(idx => String(idx.name));
  ...
  describe('2. upsertSession 与 getSession: 基础元数据读写与部分更新', () => {
    ...
    it('当会话已存在时，upsertSession 应更新指定字段且不覆盖未提供的旧字段', async () => {
    ...
    it('查询不存在的会话 ID 时应返回 null', async () => {
      const result = await sessionRepo.getSession('non_existent_session_id');
      expect(result).toBeNull();
    });
  });

  describe('3. 关联 org_employees 员工档案与外键级联约束', () => {
    it('应成功关联会话与员工 UID', async () => {
      // 1. 先在组织架构表中插入员工
      ...
      const session = await sessionRepo.getSession('ses_emp_1001');
      expect(session?.employeeId).toBe('emp_1001');
    });

    it('当关联员工被删除时，外键 ON DELETE SET NULL 应自动将会话 employee_id 置空', async () => {
      // 1. 插入部门与员工
      await orgRepo.syncOrganization({
      ...
      const sessionBefore = await sessionRepo.getSession('ses_temp');
      expect(sessionBefore?.employeeId).toBe('emp_temp');
      ...
      const sessionAfterDelete = await sessionRepo.getSession('ses_temp');
  ...
  describe('4. setTakeoverUntil 与 isTakeoverActive: 人工退避状态机', () => {
    it('setTakeoverUntil 应正确设置退避截止时间戳并刷新 updatedAt', async () => {
      ...
      await sessionRepo.setTakeoverUntil(sessionId, takeoverTimestamp);
    ...
    it('isTakeoverActive: 截止时间大于当前时间应判定为活跃 (true)，过期或为 0 应判定为不活跃 (false)', async () => {
      ...
      // 1. 在截止时间之前（退避生效中）
      expect(await sessionRepo.isTakeoverActive(sessionId, now)).toBe(true);
      expect(await sessionRepo.isTakeoverActive(sessionId, now + 10 * 60 * 1000)).toBe(true);

      // 2. 正好到达截止时间（退避结束）
      expect(await sessionRepo.isTakeoverActive(sessionId, now + 15 * 60 * 1000)).toBe(false);

      // 3. 超过截止时间（退避已结束）
      expect(await sessionRepo.isTakeoverActive(sessionId, now + 20 * 60 * 1000)).toBe(false);

      // 4. 重置为 0 后立即失效
      await sessionRepo.setTakeoverUntil(sessionId, 0);
      expect(await sessionRepo.isTakeoverActive(sessionId, now)).toBe(false);
    });

    it('对不存在的会话判定 isTakeoverActive 应安全返回 false', async () => {
      expect(await sessionRepo.isTakeoverActive('non_existent_id')).toBe(false);
    });
  });

  describe('5. touchMessageTime 与 touchReplyTime: 时间戳更新', () => {
    it('touchMessageTime 应更新 lastMessageAt 并刷新 updatedAt', async () => {
      ...
      await sessionRepo.touchMessageTime(sessionId, msgTime);
    ...
    it('touchReplyTime 应更新 lastReplyAt 并刷新 updatedAt', async () => {
      ...
      await sessionRepo.touchReplyTime(sessionId, replyTime);
    ...
    it('若不传 now 参数，touchMessageTime 和 touchReplyTime 应默认使用 Date.now()', async () => {
      const sessionId = 'ses_touch_default';
      await sessionRepo.upsertSession({ id: sessionId });

      const before = Date.now();
      await sessionRepo.touchMessageTime(sessionId);
      const after = Date.now();

      const session1 = await sessionRepo.getSession(sessionId);
      ...
      await sessionRepo.touchReplyTime(sessionId);
      const afterReply = Date.now();

      const session2 = await sessionRepo.getSession(sessionId);
  ...
  describe('6. isNewTurn: 2 小时闲置轮次切断判定', () => {
    it('当会话不存在或从未有收发消息时，应判定为新轮次 (true)', async () => {
      expect(await sessionRepo.isNewTurn('unknown_session')).toBe(true);

      const emptySessionId = 'ses_never_spoken';
      await sessionRepo.upsertSession({ id: emptySessionId });
      expect(await sessionRepo.isNewTurn(emptySessionId)).toBe(true);
    });

    it('当最后活跃时间（消息或回复）距离当前时间超过 2 小时，应判定为新轮次 (true)', async () => {
      ...
      expect(await sessionRepo.isNewTurn(sessionId, 2, now)).toBe(true);
      ...
      expect(await sessionRepo.isNewTurn(sessionId, 2, now)).toBe(false);
    });

    it('当最后活跃时间在 2 小时以内时，应判定为同一轮次延续 (false)', async () => {
    ...
    it('支持自定义 timeoutHours 超时时间', async () => {
      ...
      expect(await sessionRepo.isNewTurn(sessionId, 0.5, now)).toBe(true);

      // 自定义 1 小时超时 (60 分钟) -> 未超时 (false)
      expect(await sessionRepo.isNewTurn(sessionId, 1, now)).toBe(false);
    });

    it('当系统时间异常或未来时间戳时，应安全判定为未超时 (false)', async () => {
  ...
  describe('7. KKBotStore 统一门面与 store.sessions 调用', () => {
    it('通过 KKBotStore 门面可以正常访问 store.sessions 与 store.org', async () => {
    ...
    it('多次使用相同字段组合更新会话时应正确生效', async () => {
      ...
      await sessionRepo.upsertSession({ id: sid, name: '初始名称', mode: 'auto' });
