...
### PR267 双审查后的用户要求与实施顺序

用户要求：Required 必须先通过真实 KK9 定向核验，真实场景成立后才规划修复；Optional 仅在确有收益时优化。两项分开处理，不用模拟结果越过真机前提。

修复前真机尝试已核对本工作区授权与真实Bot5761、员工3585、私聊0-3585/int2024，启动前无其他Hook/binding。等待标记T23-267-A及五秒内附件五分钟，未收到目标入站，退出1；这不是证明问题不存在。自有Driver断开、Hook/binding消失、登录UID不变，随机库kairo_t19_8611712f363548a08154576106878cba已删除并以精确库名只读确认无残留。Required的finishing失败链暂不修改，等待用户准备好后再次启动真机监听。

Optional的实际收益是收敛两处完全相同的批次成员SQL、排序及映射，减少双处维护，不宣称降低数据库耗时。实施限于store.ts：增加同文件私有readBatchMessages(connection,batchId)，事务内传原PoolClient，公共读取传原Pool；不新增事务、连接或通用查询框架，不改SQL与公开返回合同。使用既有collector-recovery及private-chat-store真实PostgreSQL回归验证顺序、原文、超限判断及事务原子性，并运行App类型/构建和该文件静态检查；无需增加断言代码转发的测试。完成后单独提交到现有PR，明确Required仍受真机门禁阻塞。

Optional已实现并实际通过上述调用路径的真实PostgreSQL70项、App类型/构建及单文件ESLint/Prettier。六个有记录的本轮自建库精确查询无残留。生产变更仅为SQL复用，不修Required，不声称真机收尾缺陷已修复；真机监听已超时退出且自有资源回收，等待用户准备好后重新启动。

### 2026-09-10 真机证据成立后的 Required 修复计划

真实员工3585发送T23-267-B文字135973557和文件135973565，两条通过T22并进入同批97256698-712c-4499-9a0c-e439c3814803。首次收尾受控异常后两次accept收到同一错误、提示0次；显式recover才经真实T21/KK9发送135973573，delivered且真实历史可见，sendCalls=1，随后成功撤回。该证据为真实收发加一次受控异常，不声称自然数据库故障；运行退出0，身份与Hook清理正常，修复前真机门禁成立。

1. 扩展旧快照并发单元回归为前序成功/失败两种路径；新增真实PostgreSQL集成回归，要求首调用保留原异常、附件调用自行成功、无需recover即可送达一次、原文保留且无任务。
2. 先运行新增失败分支确认失败，再只修改processBatch的Promise接续：前序成功或失败都执行后继finish。保留同批串行、原调用异常、重新读库、T21幂等、allSettled和close合同；不引入自动重试、不改变send/recover、不删除整个并发层来扩大本次修改。
3. 运行定向单元、collector-recovery真实集成、App类型/构建、相关静态检查及默认App测试；继续使用自建库，不修改Driver。
4. 同一真机脚本用after模式和新标记复验：首次异常仍可见，附件提示在显式恢复前即真实送达；重复recover不增发送，实际历史和撤回均核对。
5. 真机复验后核对自建库/Hook清理，移除一次性探针，补充开发文档及现有PR/issue证据。未经授权不合并、不关闭issue。

计划已按顺序完成：两项新增失败分支先红后绿；定向单元25、App默认328、真实PostgreSQL恢复24项及类型/构建/静态检查通过。C轮两条消息不同批，门禁正确失败；D轮真实文字135974677与文件135974679间隔431ms进入同批，首次异常保留，提示135974683无需recover即送达，重复恢复不重发、无任务，真实历史与撤回通过。六个精确自建库只读核验无残留，Hook/binding已清理。生产修复仅同批Promise接续，未扩展重试、兼容或并发框架；临时探针删除，完整证据见开发文档末节。

## 21. T25 同会话队列、执行并发与时间通知（2026-09-10）

任务来源为 #230；已读正文及空评论列表。用户确认所有前置完成，当前基线具备 T19/T21/T22/T23/T24/T27，不重复验收、不合并依赖。用户已在本终端批准以下衔接合同，并配置本工作区 `.env`。

### 状态与时间合同

- 同 Bot/session 按先后处理，queued 上限来自 perSessionQueue（默认3）；running、waiting_for_user、ready_to_send、sending 均阻塞后续。completed/failed/cancelled/timed_out/send_unconfirmed 无出边。
- 全局默认3个实际执行；任务终态不代表底层退出。真实执行及 Tool 必要收尾结束后才释放名额；无法立即取消时保留在途计数，退出自动唤醒，永久不退出明确诊断并交人工处理。不伪造 running，不突破并发，不新增进程隔离或自动重启。
- 员工同意先持久化，拿到全局名额才恢复原任务 running 与剩余执行预算；等员工与等名额不扣预算，已及时回答不能因随后等位被误判等待超时。
- 原始执行预算持久保存；进度按预算减当前剩余累计，必要收尾计时，等待暂停。9.999秒与恰好10秒不提示，10.001秒仍执行才提示，同task唯一progress purpose。
- queueDeadline沿用批次原始结束时刻加queueMs；首次领取设置executionDeadline，恢复仅使用原剩余。截止等号即到期。执行到期触发实际AbortController，采用前再比较绝对时间；已sending由T21处理，不用执行超时推断发送失败。
- 满队列裁决与入队在context锁内完成，持久保留queue_full拒绝，原始消息不删且不建task。T23重放不重新接纳被拒批次，不复活discarded；queued/progress及各固定提示使用T21独立稳定purpose。

### 实施顺序与接口

1. 扩展现有TaskStore的原子enqueueTask、有效非终态读取、已同意等待的resumeTask；保留createTask的底层账本用途，正式Collector全部改为调度交付。新增000012仅包含实际需要的原始预算、排队提示与当前等待指针、批次拒绝约束和查询索引；claim/resume同事务返回任务快照。
2. TaskExecutor必须显式提供，输入真实task/attempt/context及signal，返回已检查正文或员工等待问题；执行Promise只在Agent和Tool收尾完成后结束。T25不装配T28 Agent、不实现T29答案审核、不提供默认替身。
3. TaskRunner负责attempt登记、累计进度、绝对截止、采用及T21异步发送；调度器以进程内实际执行集合限制并发，发送回执与任务状态变化唤醒后续调度，不持数据库锁等待外部请求。
4. Collector通过必需deliverReady接口交付ready批次和既有queue_full拒绝；去掉原创建任务接入，不保留可选兼容路径。T24/new仍直接消费，不排在旧执行后面。

### 验收计划

- 单元覆盖同会话2/4/5任务、三会话并行第四等待、9.999/10/10.001秒、排队与执行恰好截止、等待预算暂停及恢复、/new、ready/sending阻塞、终态释放、取消未退出与迟到结果。
- 真实PostgreSQL沿用createTaskTestDatabase，自建随机库，核对库名和双连接PID后迁移；覆盖并发满队列裁决、同批唯一、同会话领取、/new与领取/恢复竞争、及时回答等槽不损预算、终态及迟到采用。
- 运行本次精确定向单元/集成、受影响既有调用回归、迁移专题、App类型/构建及相关静态检查。独立普通Node/tsx实际运行原生时间调度路径，明确执行器受控、出站FakeDriver，不冒称真实Agent或T35双员工慢查询验收。
- 最后审查复杂度、清理一次性探针、更新实际证据和Orca comment。未获进一步授权不合并分支、不关闭issue，不访问真实KK9或操作其他工作区进程/数据库。

### 实施与审查结果

实现已落地；同毫秒等待指针、关闭漏等、暂停进度与旧等待交付等反例均有失败/修复验证。精确单元101项、受影响真实PostgreSQL185项、迁移专题2项通过，App类型/构建及lint通过。根链曾遇App IPC关闭，诊断参数下原样重跑App368项通过，不声称修复测试框架。
